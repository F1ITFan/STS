...
public class CertifiableItemMatcher extends JavaMatcher {
   ...
   private SailPointContext context = getContext();
      ...
      public Object getPropertyValue(LeafFilter leaf, Object o)
         ...
         if (property != null) {
            if (property.startsWith("ma.") && ent.getAttributes() != null) {
               ...
               ItemToObjectMatcher itemToObjectMatcher = new ItemToObjectMatcher(context);
               ...
               if (("value".compareTo(property) == 0)) {
                  ...
                  && ent.getAttributes() != null) {
               // If we want to get values directly from the account,
               // we should specify link.<attribute>
               property = property.substring(5);
               val = getValFromLink(property, ent.getNativeIdentity(), ent.getApplicationName()); 
   ...
   private Object getValFromLink(String property, String nativeIdentity, String appName) throws GeneralException {
       Link link = null;
       Object val = null;
       ...
       if (null != it && it.hasNext()) {
           // There should only be one link for a given application with
           // a given nativeIdentity
           link = (Link) it.next();
           if (("displayName".compareTo(property) == 0)) {
              val = link.getDisplayName();
           } else if (("identity".compareTo(property) == 0)) {
              val = link.getNativeIdentity();
           } else if (("nativeIdentity".compareTo(property) == 0)) {
              val = link.getNativeIdentity();
           } else if (("application".compareTo(property) == 0)) {
              val = link.getApplicationName();
           } else if (("application.name".compareTo(property) == 0)) {
              val = link.getApplicationName();
           } else if (("application.id".compareTo(property) == 0)) {
              val = link.getApplicationId();
           } else if (("uuid".compareTo(property) == 0)) {
              val = link.getUuid();
           } else if (("locked".compareTo(property) == 0)) {
              val = link.isLocked();
           } else if (("manuallyCorrelated".compareTo(property) == 0)) {
              val = link.isManuallyCorrelated();
           } else if (("disabled".compareTo(property) == 0)) {
              val = link.isDisabled();
           } else if (("class".compareTo(property) == 0)) {
              val = link.getClass().getSimpleName();
           } else {
              val = (link.getAttributes() != null) ? link.getAttributes().get(property) : val;
           }
        }
        return val;
