 ...
 *  
 *  27 July 2017 - Fixed typo attribute name "identityFilter"
 *  
 *  07 Sept 2017 - Removed logging schema to the Console.
 * 
 *  23 May 2018  - Added Authenticate method for pass-through and
 *  			   password self service.  Thanks to Menno for the suggestion
 *  
 *  24 May 2018  - Fixed issue where hashed passwords resulted in authentication method
 *  			   always failing.
 *  
 * */
public class LoopbackConnector extends AbstractConnector {
	...
	public final static String APP_ATTR_IDENTITY_FILTER = "identityFilter";
	...
	/**
	 * Authenticate method used for pass through authentication and enabling
	...
	public ResourceObject authenticate(String username, String password) throws ConnectorException,
		...
		if (username == null || password == null || username.length() == 0 || password.length() == 0) {
			throw new AuthenticationFailedException("Username or password cannot have empty value");
		}
		
		ro = getObject("account", username, null );
	
		if (ro != null) {
			// We have a resource object.. so lets try and authenticate
			
		
			
			Identity id = null;
			try {
				id = context.getObjectByName(Identity.class, username);
			} catch (GeneralException e) {
				throw new AuthenticationFailedException(MessageKeys.AUTH_FAILURE_INVALID_PASSWD);
			}

			// Respect any security stuff used in IdentityIQ and check authentication...
			
			Boolean enableAuthLockout = false;
			Boolean isIdentityLocked = false;
			Boolean protectedUserLockout = false;
			int maxLoginAttempts = 0;
			
			Lockinator lockinator = new Lockinator(context);
			
			try {
				
				enableAuthLockout = context.getConfiguration().getBoolean(Configuration.ENABLE_AUTH_LOCKOUT);
				isIdentityLocked = lockinator.isUserLocked(id, Lockinator.LockType.Login);	
				maxLoginAttempts = context.getConfiguration().getInt(Configuration.FAILED_LOGIN_ATTEMPTS);
				protectedUserLockout = context.getConfiguration().getBoolean(Configuration.PROTECTED_USER_LOCKOUT);
						
			} catch (GeneralException e) {
				log.warn("Auth lockout not available");
			}

			if(isIdentityLocked && enableAuthLockout) {
				throw new AuthenticationFailedException( new IdentityLockedException(MessageKeys.IDENTITY_LOCKED_OUT_ERROR));
		    }
			
			boolean isMatch = EncodingUtil.isMatch(password, id.getPassword(), context);
	        if (!isMatch) {

	            int failedattempts = id.getFailedLoginAttempts() + 1;
	            id.setFailedLoginAttempts(failedattempts);
	            
	            if((failedattempts >= maxLoginAttempts) && enableAuthLockout) {
	                if(protectedUserLockout) {
	                    
	                    try {
							lockinator.lockUser(id, id.getName(), true);
							
						} catch (GeneralException e) {
							log.error("User lock exception", e);
						}
	                    
	                } else if(!id.isProtected()) {
	                    
	                    try {
							lockinator.lockUser(id, id.getName(), true);
						} catch (GeneralException e) {
							log.error("User lock exception on unprotected identity", e);
						}
	                }
	            } else {
	                ...
	                try {
						...
						log.error("Error on saving object", e);
					}
			...
			try {
				PasswordPolice passwordPolice;
				passwordPolice = new PasswordPolice(context);
				passwordPolice.checkExpiration(id);
			} catch (GeneralException e) {
	...
	private Filter getIdentityIQObjectFilter(Schema schema) {
		...
		try {
			if (log.isDebugEnabled()) log.debug("Schema: " + schema.toXml());
